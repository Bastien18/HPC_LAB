...
// ====================== VERSION OPTIMISEE MANUELLEMENT 1 ======================
/*uint16_t filter_pixel_1D(const struct img_1D_t *img, const int16_t *kernel, size_t kernel_size, size_t x, size_t y)
{
    uint16_t val = 0;
    ...
    for (size_t i = 0; i < kernel_size; ++i)
    {
        size_t pixel_y = y - half_kernel + i;
        size_t img_offset = pixel_y * img->width;

        for (size_t j = 0; j < kernel_size; ++j)
        {
            size_t pixel_x = x - half_kernel + j;

            val += img->data[img_offset + pixel_x] * kernel[i * kernel_size + j];
        }
    }
...
// ====================== VERSION OPTIMISEE MANUELLEMENT 2 ======================
/*uint16_t filter_pixel_1D(const struct img_1D_t *img, const int16_t *kernel, size_t kernel_size, size_t x, size_t y)
{
    uint16_t val = 0;
    size_t half_kernel = kernel_size / 2;
    ...
    {
        size_t pixel_y = y - half_kernel + i;
        const uint8_t *img_row = &img_data[pixel_y * img_width];

        for (size_t j = 0; j < kernel_size; ++j)
        {
            size_t pixel_x = x - half_kernel + j;
            val += img_row[pixel_x] * kernel[i * kernel_size + j];
        }
    }
...
uint16_t filter_pixel_1D(const struct img_1D_t *img, const int16_t *kernel, size_t kernel_size, size_t x, size_t y)
{
    ...
    if (kernel_size != 3)
    {
        printf("Error kernel size must be 3\n");
        return -1;
    }

    uint16_t temp_pixels[16] __attribute__((aligned(32)));
    ...
    for (size_t i = 0; i < kernel_size; ++i)
    {
        ...
        for (size_t j = 0; j < kernel_size; ++j)
        {
            ...
            // Chargement du pixel de l'image dans le vecteur
            temp_pixels[j] = img->data[img_offset + pixel_x];
        }

        // Chargement du tableau temporaire dans un vecteur SIMD
        __m256i img_pixels = _mm256_load_si256((__m256i *)temp_pixels);

        // Chargement du noyau dans un vecteur
        __m256i kernel_values = _mm256_loadu_si256((__m256i *)&kernel[i * kernel_size]);

        // Multiplication verticale des pixels de l'image par les éléments du noyau
        __m256i mul_result = _mm256_mullo_epi16(img_pixels, kernel_values);

        /*for (int i = 0; i < kernel_size; ++i)
        {
            val += _mm256_extract_epi16(mul_result, i);
        }*/
        val += _mm256_extract_epi16(mul_result, 0);
        val += _mm256_extract_epi16(mul_result, 1);
        val += _mm256_extract_epi16(mul_result, 2);
    }
...
uint16_t filter_pixel_chained(struct pixel_t **right_border_px, const int16_t *kernel, size_t kernel_size)
{
    ...
    // Vecteur pour stocker les résultats de la multiplication verticale
    ...
    for (size_t i = 0; i < kernel_size; ++i)
    {
        ...
        for (size_t j = 0; j < kernel_size; ++j)
        {
            temp_pixels[j] = actual_px->pixel_val[0];
            actual_px = actual_px->next_pixel;
        }
